...
async def add_monitor_playlist(request: Request) -> dict[str, Any]:
    ...
    if parsed is None or parsed[0] not in ('playlist', 'artist'):
        raise HTTPException(
            status_code=400, detail='A valid Spotify playlist or artist URL is required'
        )
    ...
    if existing is not None:
        type_str = 'artist' if existing.type == 'artist' else 'playlist'
        raise HTTPException(
            status_code=409, detail=f'This {type_str} is already being monitored'
        )
    ...
    if kind == 'artist':
        ...
        playlist = await asyncio.to_thread(
            db.add_playlist, spotify_id, name, url, interval_minutes, type='playlist'
        )
    ...
    async def _run() -> None:
        try:
            if playlist.type == 'artist':
                ...
                logger.info(
                    'Manual check: downloaded {} new track(s) from playlist "{}"',
                    count,
                    playlist.name,
                )
        except Exception:
            logger.exception(
                'Manual check failed for {} {}', playlist.type, playlist_id
            )
