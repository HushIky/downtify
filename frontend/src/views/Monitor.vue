      ...
      <div class="surface rounded-2xl p-5 mb-8">
        ...
        <form @submit.prevent="onAdd" class="flex flex-col sm:flex-row gap-3">
          <input
            v-model="newUrl"
            type="text"
            :placeholder="t('monitor.urlPlaceholderMixed')"
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <span
                v-if="pl.type === 'artist'"
                class="pill shrink-0 bg-purple-500/15 text-purple-400 border border-purple-500/20"
              >
                {{ t('monitor.typeArtist') }}
              </span>
              <span
                v-else
                class="pill shrink-0 bg-primary/15 text-primary border border-primary/20"
              >
...
async function load() {
  loading.value = true
  try {
    const plRes = await monitorAPI.listMonitoredPlaylists()
...
async function onChangeInterval(item, event) {
...
async function onDelete(item) {
  const promptMsg = item.type === 'artist'
